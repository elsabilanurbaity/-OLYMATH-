...
def buat_template_excel():
    ...
    ws["A2"] = "Isi data siswa mulai baris ke-5. Semua nilai dalam rentang 0–100. JANGAN ubah nama kolom baris ke-4."
    ws["A2"].font = Font(italic=True, size=9, color="FFFFFF")
    ws["A2"].fill = PatternFill("solid", fgColor="4338CA")
    ws["A2"].alignment = Alignment(horizontal="center", vertical="center", wrap_text=True)
    ws.row_dimensions[2].height = 28

    # Baris 3: Nama panjang (keterangan saja, warna lebih muda)
    keterangan = [
        "Nama Siswa", "Numerasi Aljabar", "Numerasi Geometri",
        "Numerasi Bilangan", "Data & Ketidakpastian", "Kemampuan Menalar", "Literasi Matematika"
    ]
    ket_colors = ["93C5FD", "C4B5FD", "93C5FD", "67E8F9", "6EE7B7", "FCD34D", "FCA5A5"]
    for c_idx, (ket, color) in enumerate(zip(keterangan, ket_colors), 1):
        cell = ws.cell(row=3, column=c_idx, value=ket)
        cell.font = Font(italic=True, size=9, color="1E1B4B")
        cell.fill = PatternFill("solid", fgColor=color)
        cell.alignment = Alignment(horizontal="center", vertical="center", wrap_text=True)
        cell.border = border
    ws.row_dimensions[3].height = 30

    # Baris 4: KODE KOLOM (ini yang dibaca oleh sistem – WAJIB TEPAT)
    # Kolom harus persis: Nama Siswa, NUM_ALJ, NUM_GEO, NUM_BIL, NUM_DAT, NUM_L3, LIT
    col_codes = ["Nama Siswa", "NUM_ALJ", "NUM_GEO", "NUM_BIL", "NUM_DAT", "NUM_L3", "LIT"]
    header_colors = ["1E40AF", "7C3AED", "2563EB", "0891B2", "059669", "D97706", "DC2626"]
    for c_idx, (code, color) in enumerate(zip(col_codes, header_colors), 1):
        cell = ws.cell(row=4, column=c_idx, value=code)
    ...
    for r_idx, row_data in enumerate(samples, 5):
    ...
    # Baris 8-57: Baris kosong untuk input
    for r in range(8, 58):
        for c in range(1, 8):
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    if uploaded:
        try:
            # Coba baca sebagai template OLYMATH (header di baris ke-4, index 3)
            # sekaligus fallback ke baca biasa jika bukan template OLYMATH
            df_try = pd.read_excel(uploaded, header=3)  # baris ke-4 (index 3) = kode kolom
            cols_try = [str(c).strip() for c in df_try.columns]

            required = set(features + ["Nama Siswa"])
            if required.issubset(set(cols_try)):
                # Ini template OLYMATH – header di baris 4
                df_input = df_try.copy()
                df_input.columns = cols_try
                # Buang baris yang semua kosong
                df_input = df_input.dropna(how="all")
                # Buang baris contoh jika ada (cek apakah nilai NUM_ALJ bisa dikonversi ke float)
                def is_numeric_row(row):
                    try:
                        ...
                        return False
                df_input = df_input[df_input.apply(is_numeric_row, axis=1)].reset_index(drop=True)
            else:
                # Bukan template OLYMATH – baca normal dari baris pertama
                uploaded.seek(0)
                df_input = pd.read_excel(uploaded)
                df_input.columns = [str(c).strip() for c in df_input.columns]

                # Normalisasi nama kolom panjang → kode kolom
                rename_map = {
            ...
            st.success(f"✅ File berhasil dibaca: **{len(df_input)} baris** data siswa terdeteksi.")
            ...
            if missing:
                st.error(
                    ...
                    "Pastikan kamu menggunakan **Template Excel OLYMATH** yang sudah disediakan, "
                    "dan **jangan mengubah nama kolom baris ke-4** pada template."
                )
                st.stop()
